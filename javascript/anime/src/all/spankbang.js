...
class DefaultExtension extends MProvider {
  ...
  // ❌ first 8 are the featured row → drop them
  if (elements.length > 8) {
    elements = elements.slice(8);
  }
  ...
  // SpankBang returns ~24 paginated results per page after featured
  const hasNextPage = items.length >= 24;
  ...
  // ❌ drop first 8 featured/pinned items
  if (elements.length > 8) {
    elements = elements.slice(8);
  }
  ...
  // SpankBang shows ~24 real results per page after featured
  const hasNextPage = items.length >= 24;
  ...
  const title =
    doc.selectFirst("h1.main_content_title")?.text?.trim() ??
    "Untitled";
  ...
  const episodes = [
    {
      name: title, // episode name same as title
      url: url     // single episode points to the same page
    }
  ];
  ...
  // Match the stream_data JSON
  const match = body.match(/var\s+stream_data\s*=\s*(\{.*?\});/s);
  if (match) {
    let jsonStr = match[1];

    // fix for JS → JSON (single quotes → double quotes)
    jsonStr = jsonStr
      .replace(/'/g, '"')
      .replace(/,\s*}/g, "}")
      .replace(/,\s*]/g, "]");
